...
int main(int argc, char*argv[]) {
  ...
  // Allocate 4 bytes
  i = (int *)allocate(4, 0);  // size of an int is 4 bytes
  *i = 10;
  printf("value of int i %d\n", *i);
  
  // Allocate 1 byte
  c = (char*)allocate(1, 0);  // size of char is 1 byte
  *c = 'H';
  ...
  d = (char*)allocate(1, 0);  // size of char is 1 byte
  *d = 'E';
  printf("value of char* d %c\n", *d);
  
  s = (char*)allocate(10, 0);  // 10 characters long
  strncpy(s, "Hello!", 7);
  printf("value of char* s '%s'\n", s);

  x = (char*)allocate(9, 0);  // 9 characters long
  strncpy(x, "World!", 7);
  printf("value of char* x '%s'\n", x);

  y = (char*)allocate(8, 0);  // 8 characters long
  strncpy(y, "TEST!", 7);
  ...
  z = (char*)allocate(8, 0);  // 10 characters long
  strncpy(z, "YES!", 7);
  printf("value of int z '%s'\n", z);
  
  n = (struct node *)allocate(sizeof(struct node), 0);
  n->v = (char*)allocate(20, 0);
  n->count = 3;
  ...
  // SHOULD BE DIFFERENT FOR (first fit vs. best fit)
  a = (char*)allocate(2, 0);  // 10 characters long
  strncpy(a, "a", 2);
