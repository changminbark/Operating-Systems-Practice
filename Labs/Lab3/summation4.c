...
void *SumExp(void *args_ptr) {
	...
	printf("Thread %d had EXECUTION TIME: %lf seconds\n", myargs_ptr->tid, elapsed);

	char *curr_time = ctime(&tv_end.tv_sec);
	printf("Thread %d had CURRENT TIME: %s\n", myargs_ptr->tid, curr_time);
